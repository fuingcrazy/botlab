...
private:
    float pid[3] = {1.0, 6.0, 0.0}; //kp, ka, kb
...
class LQR_Controller : public ManeuverControllerBase
{
    private:
      mat3x3 Q;
      mat2x2 R;
      mat3x3 A;
      mat3x2 B;    //define lqr matrix
      float_t t = 0.2;
      std::vector<double> control = {0.01,0};    //[v,psi],set initial control value(not all zero for Riccati solving)
    public:
      LQR_Controller() = default;
      virtual mbot_lcm_msgs::twist2D_t get_command(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target) override
      {
        A<<1.0, 0.0, -control[0]*t*sin(pose.theta),
           0.0, 1.0, control[0]*t*cos(pose.theta),
           0.0, 0.0, 1;
        B<<t*cos(pose.theta),0,
           t*sin(pose.theta),0,
           0,                t;
        Q<<20,0,0,
           0,6,0,
